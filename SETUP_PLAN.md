...
- `api/client.ts` — axios instance (baseURL из env), request-интерсептор (Authorization из useAuthStore.getState()), response-интерсептор: нормализация ошибки в `ApiError { code, status, message, details }` (класс в `types/api.ts`) + каркас refresh-single-flight (очередь ожидающих запросов, флаг `_retry`; сам refresh-эндпоинт — TODO до появления бэка, структура готова).
...
- `api/queries/useUserProfileQuery.ts` + `api/queries/useUpdateProfileMutation.ts` — примеры обёрток (мутация — с `meta.successToast`).
...
- `app/providers/QueryProvider.tsx` — QueryClient c `QueryCache({ onError })` и `MutationCache({ onError, onSuccess })`: тосты через react-toastify, локализация `t('errors.' + error.code, { defaultValue: t('errors.generic') })`, поддержка `meta.suppressErrorToast` / `meta.successToast` / `meta.invalidates` (массив queryKey → invalidateQueries в onSuccess). Типизация meta через `declare module '@tanstack/react-query' { interface Register { mutationMeta: ..., queryMeta: ... } }`. Devtools за `env.VITE_ENABLE_DEVTOOLS`.
